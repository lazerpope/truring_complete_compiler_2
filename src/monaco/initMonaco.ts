...
const THEME_ID = 'andromeda-bordered'
  ...
  const configureMonaco = (): monaco.IDisposable => {
    ...
    monaco.languages.setMonarchTokensProvider(LANGUAGE_ID, {
      ...
      tokenizer: {
        root: [
          ...
          [/(Math)(\.)(min|max|smin|smax|abs)(?=\s*\()/, ['namespace', 'delimiter', 'function']],
          [
          ...
          [/([A-Za-z_$][\w$]*)(\.)([A-Za-z_$][\w$]*)\b/, ['identifier', 'delimiter', 'property']],
    ...
    monaco.editor.defineTheme(THEME_ID, {
      ...
      rules: [
        ...
        { token: 'comment', foreground: 'A0A1A7CC' },
        { token: 'keyword', foreground: 'C74DED' },
        { token: 'constant.language', foreground: 'EE5D43' },
        { token: 'constant.numeric', foreground: 'F39C12' },
        { token: 'namespace', foreground: 'F39C12' },
        { token: 'type.identifier', foreground: 'EE5D43' },
        { token: 'function', foreground: 'FFE66D' },
        { token: 'property', foreground: 'F39C12' },
        ...
        { token: 'identifier', foreground: '00E8C6' },
    ...
    const snippets: CompletionTemplate[] = [
      ...
      {
        label: { label: 'class', description: 'static struct schema' },
        filterText: 'class',
        kind: monaco.languages.CompletionItemKind.Snippet,
        detail: 'Precompiler-only static struct schema',
    ...
    return monaco.languages.registerCompletionItemProvider(LANGUAGE_ID, {
      ...
      provideCompletionItems(currentModel, position) {
        ...
        if (/\bMath\.[A-Za-z_]*$/.test(linePrefix)) {
          ...
          const schemas = new Map<string, Array<{ name: string; isArray: boolean }>>()
          for (const classMatch of documentText.matchAll(
            /^\s*class\s+([A-Za-z_$][\w$]*)\s*\{\s*$([\s\S]*?)^\s*\}/gm,
          )) {
            const fields: Array<{ name: string; isArray: boolean }> = []
            for (const fieldMatch of (classMatch[2] ?? '').matchAll(
              /^\s*([A-Za-z_$][\w$]*)\s*=\s*(.+)$/gm,
            )) {
              fields.push({
                name: fieldMatch[1] ?? '',
                isArray: /^\s*(?:Array\s*\(|\[)/.test(fieldMatch[2] ?? ''),
              })
            }
            schemas.set(classMatch[1] ?? '', fields)
          }
          const instanceClasses = new Map<string, string>()
          for (const instanceMatch of documentText.matchAll(
            /^\s*(?:let|const|var)\s+([A-Za-z_$][\w$]*)\s*=\s*([A-Za-z_$][\w$]*)\s*\(/gm,
          )) {
            if (schemas.has(instanceMatch[2] ?? '')) {
              instanceClasses.set(instanceMatch[1] ?? '', instanceMatch[2] ?? '')
            }
          }
          const structFields = schemas.get(instanceClasses.get(accessedName ?? '') ?? '') ?? []
          templates = structFields.map((field) => ({
            label: field.name,
            kind: monaco.languages.CompletionItemKind.Property,
            detail: field.isArray ? 'Static struct array field' : 'Static struct scalar field',
          ...
          const classCompletions: CompletionTemplate[] = []
          for (const classMatch of documentText.matchAll(/^\s*class\s+([A-Za-z_$][\w$]*)\s*\{/gm)) {
            const label = classMatch[1] ?? ''
            classCompletions.push({
              label,
              kind: monaco.languages.CompletionItemKind.Constructor,
              detail: 'Static struct constructor',
              insertText: `${label}($1)`,
              insertTextRules: snippetRule,
            })
          }
